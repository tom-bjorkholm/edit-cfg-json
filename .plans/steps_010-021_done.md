...
in `doc/detailed_design.md` where each of them belongs.
...
be changed in `./doc/detailed_design.md` in docstrings and in the example descriptions.
...
`doc/detailed_design.md` and this plan. `doc/*_api.md` and the three `README_pypi.md`
...
- **`doc/detailed_design.md` never wrote `--ui dump` at all**, and its sections 4 to 7
...
Remove references to step numbers in `./doc/detailed_design.md` and `readme_parts`
...
The future reader of `./doc/detailed_design.md` or `README_pypi` will not be
...
- **The step numbers go from everywhere except `./.plans`.** The plan is the
  one document whose subject really is the order of the work. `doc/detailed_design.md`
...
**Core.** No code at all: `doc/detailed_design.md`, the seven root `readme_parts/`
...
Designed in full in `doc/detailed_design.md` section 8.2. The design was recorded
...
**What it decided.** Four things, decided before the work started and recorded
in `doc/detailed_design.md` sections 9.8, 9.9 and 8.3.2.
...
- **Every setting reaches a program through a settings file and through
  ...
  `Settings` grows an attribute, which section 9.1 of `doc/detailed_design.md` promises
...
`doc/detailed_design.md` gains section 8.3.5 and one paragraph of 9.9,
...
- **What the design asks for is all there.** Checked rather than assumed:
  ...
  `add_file_options` are every option `doc/detailed_design.md` names for a program, and
...
## 6. Open questions recorded, and now answered
...
| --- | --- |
| ~~When does a field report that its text means no value at all, and is that on focus loss?~~ Answered at step 7: it does, on focus loss, through `EditModel.check_field`. See `doc/detailed_design.md` sections 4.2 and 6.5. | done |
...
| ~~Which widget does the Tk backend bind its keys on when it shares a window?~~ Answered at step 18: a bind tag of the editor's own, on the widget it was built below and everything inside it, which is one rule for a window the editor owns and for one it shares. See `doc/detailed_design.md` section 8.2.7. | done |
