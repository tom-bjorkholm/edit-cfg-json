...
The four examples that mount the editor in a window an application owns cannot
use `run_example` at all: every option of it is phrased against
`edit_cfg_json.EditorBackend`, and a mounted editor is deliberately not one —
it does not run to completion, so it cannot be handed to `edit()`. Their own
command line is in `_shared_pipeline`, beside the configuration all four of
them edit.
"""
