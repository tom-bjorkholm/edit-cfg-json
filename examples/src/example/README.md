...
| `-i`, `--input` | Configuration file to read. Not supported yet. |
...
The two file options are accepted but refused, with a message saying so.
They exist already so that the command line does not have to change again
when reading and writing files arrive.
