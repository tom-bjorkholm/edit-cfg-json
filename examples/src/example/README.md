...
| [e11_add_remove.py](e11_add_remove.py) | Changing **how many** things a member holds: adding an element, removing one and moving one along a list. Where a new element comes from — the class a nesting declaration names, the elements the class declares for the member itself, and failing both of those the type the member is annotated with. Also the three dicts that cannot gain a key and the three different reasons why, and an optional member that is given its object and put back to holding none. |
